...
def generate_model_data(data_path, alpha, days):
    ...
    for i in range(train_day):
        Y_data.append(df['Adj Close'][i + days - 1])
        for j in range(days):
...
def lstm_model(X_train, Y_train, X_test, Y_test):
    ...
    model.add(Bidirectional(LSTM(units=20, activation='hard_sigmoid', return_sequences=False)))
...
if __name__ == '__main__':
    ...
    X_train, Y_train, X_test, Y_test = generate_model_data('AAPL_stock_data.csv', alpha, days)
